...
const API_KEY = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJZY1l6eDRCSUJNdXZlaWFUMGdxcDdoNERoUTdLaEc0MlhpSElENHI4MUlJIiwiaWF0IjoxNzA0MzM5NDU5fQ.vq-62V167_79eLdSIkdQBnhMdryKR7C1PfjpDlfh578'
...
class DmsModel {
  ...
  async createDataset(req, res) {
    ...
    client.action('dataset_create', {
        ...
        date_of_issue: req.body.date,
        citation: req.body.citation,
        type_of_scientific_work: req.body.type_of_scientific_work,
        series: req.body.series,
        advisor: req.body.advisor
    }, async function(err, result) {
        if (err) {
            console.error('Error creating dataset:', err);
            return res.redirect('/createError');
        } else {
            console.log('Dataset created:', result);
        
            // Simpan data metadata dalam sesi atau penyimpanan sesaat
            const datasetMetadata = {
                package_id: req.body.name,
                name: req.body.name,
  ...
  async createResource(req, res) {
    ...
    const resourceMetadata = {
      ...
      format: datasetMetadata.name.split('.').pop(),
      upload: '@' + upload,// Format benar untuk mengunggah file
      url:datasetMetadata.name,
      url_type:'upload'
    };
